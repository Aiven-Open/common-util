...
public class BackoffTest {
  ...
  void backoffTest() throws InterruptedException {
    ...
    assertThat(stopWatch.getDuration().toMillis())
        .as("Result without timer running")
        .isBetween(
  ...
  /**
   * This test creates the condition where the delay increases until the expected delay will cause
   * the maxDelay to be exceeded. The result is that the total delay should be the maxDelay not the
   * higher calculated expected value.
   *
  ...
  void noTimerRuleTest() throws InterruptedException {
    ...
    assertThat(stopWatch.getDuration().toMillis())
        .as("Result without timer running")
        .isBetween(
            1024 - backoff.getMaxJitter() - TIMING_DELTA_MS,
            1024 + backoff.getMaxJitter() + TIMING_DELTA_MS);
  }
