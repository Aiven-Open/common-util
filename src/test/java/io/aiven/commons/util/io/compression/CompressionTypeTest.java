...
class CompressionTypeTest {
	...
	private byte[] decompress(IOSupplier<InputStream> input, final CompressionType compressionType) throws IOException {
	...
	void roundTripIOSupplierTest(final CompressionType compressionType) throws IOException {
		...
		final byte[] decompressed = decompress(() -> new ByteArrayInputStream(compressed), compressionType);
		assertThat(new String(decompressed, StandardCharsets.UTF_8)).isEqualTo(testText);
	}
