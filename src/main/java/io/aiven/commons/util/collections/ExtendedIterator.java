...
public final class ExtendedIterator<T> implements Iterator<T> {
  /**
   * Set to <code>true</code> if this wrapping doesn't permit the use of {@link #remove()},
   * otherwise removal is delegated to the base iterator.
   */
  private final boolean removeDenied;

  /**
   * Answer an ExtendedIterator wrapped round <code>it</code>, which does not permit <code>.remove()
   * </code> even if <code>it</code> does.
   *
   * @param it The Iterator to wrap.
   * @param <T> The object type returned by the iterator.
  ...
  /**
   * Answer an ExtendedIterator wrapped round a {@link Stream}. The extended iterator does not
   * permit <code>.remove()</code>.
   *
   * <p>The stream should not be used directly. The effect of doing so is undefined.
   *
  ...
  /**
   * Answer an ExtendedIterator returning the elements of <code>it</code>. If <code>it</code> is
   * itself an ExtendedIterator, return that; otherwise wrap <code>it</code>.
   *
   * @param it The iterator to wrap.
   * @param <T> The object type returned by the iterator.
   ...
   * @return A new iterator returning the contents of {@code this} iterator followed by the contents
   *     of {@code other{ iterator.}}
  ...
  /**
   * Filter this iterator using a predicate. Only items for which the predicate returns {@code true}
   ...
   * @return An Extended iterator that returns a {@code <U>} for very {@code <T>} in the original
   *     iterator.
  ...
  /**
   * A method to add the remaining elements in the iterator an arbitrary collection. This method
