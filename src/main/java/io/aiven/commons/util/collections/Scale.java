...
public enum Scale {
  ...
  /**
   * The International Electrotechnical Commission (IEC) standardized binary prefixes. Developed by
   * the IEC to avoid ambiguity through their similarity to the standard metric terms. These are
   * based on powers of 2.
   *
   * @see <a href='https://www.iec.ch/prefixes-binary-multiples'>IEC prefixes for binary
   *     multiples</a>.
   */
  public static final List<Scale> IEC = Arrays.asList(KiB, MiB, GiB, TiB, PiB);

  /**
   * The SI standardized prefix scales. These are the metric units, as such they are all powers of
   * 10.
   */
   ...
   * Constructor.
   *
   * @param bytes the number of bytes in a single unit of the scale.
   */
   ...
   * @return A string representing the number of units that comprise the {@code byteCount}.
   */
  public String format(final long byteCount) {
   ...
   * Formats the {@code byteCount} at this scale.
   *
   * @param unitCount the number of units at this scale.
  ...
  /**
   * Determines the scale of the number of bytes. The largest Scale for which {@code byteCount}
   * represents at least 1 unit is returned. If no scale in the possible scales matches then bytes
   * are returned. Negative number will return {@link Scale#B}.
   *
   * @param byteCount the number of bytes
   * @param possibleScales the list of possible scales.
   ...
   *
   * @param byteCount the number of bytes.
   * @param possibleScales the possible Scales.
   * @return a formatted string representation of the {@code byteCount} in the best Scale
   *     representation.
  ...
  /**
   * Creates a String using the scale. if the scale is not {@link #B} then it is followed by the
   * number of bytes within a set of parenthesis.
   *
   * @param value the number of bytes.
   * @return a String using the scale. if the scale is not {@link #B} then it is followed by the
   *     number of bytes
   */
