 ...
 *
 * <p>The majority of this code is copied from Apache HttpComponents-core
 *
 * @see <a href=
...
public class HttpStrings {
  ...
  public static String getReason(final int status) {
    ...
    String reason = null;
    if (category > 0
        && category < REASON_PHRASES.length
        && subcode < REASON_PHRASES[category].length) {
  ...
  /**
   * Stores the given reason phrase, by status code. Helper method to initialize the static lookup
   * table.
   *
