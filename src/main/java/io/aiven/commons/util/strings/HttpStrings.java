...
public class HttpStrings {
	...
	private static final String[][] REASON_PHRASES = new String[][]{null, new String[4], // 1xx
			new String[27], // 2xx
			new String[9], // 3xx
			new String[52], // 4xx
			new String[12] // 5xx
	 ...
	 */
	public static String getReason(final int status) {
		final int category = status / 100;
		final int subcode = status - 100 * category;

		String reason = null;
		if (category > 0 && category < REASON_PHRASES.length && subcode < REASON_PHRASES[category].length) {
			reason = REASON_PHRASES[category][subcode];
		} else {
			reason = "Unrecognized status code: " + status;
		}
		return reason;
	}

	/**
	 * Stores the given reason phrase, by status code. Helper method to initialize
	...
	static {
		...
		setReason(HttpStatus.SC_UNPROCESSABLE_CONTENT, "Unprocessable Content");
